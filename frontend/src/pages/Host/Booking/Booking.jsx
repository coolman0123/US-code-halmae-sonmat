  ...
  const generateBookingData = () => {
    ...
    savedReservations.forEach((reservation) => {
      ...
      if (reservation.totalPrice) {
        ...
        // 할매 정보에서 가격 찾기
        const hostInfo = hostsList.find(
          (host) => host.houseName === reservation.houseName
        );
    ...
    if (savedReservations.length === 0 && year === 2025 && month === 5) {
      ...
      const defaultPrice =
        hostsList.length > 0 && hostsList[0].price
          ? parseInt(hostsList[0].price)
          : 340000;
      ...
      // 예약 가능한 날짜들 (녹색 - '가')
      const availableDays = [
        1, 4, 5, 6, 10, 11, 12, 13, 14, 17, 18, 19, 20, 21, 25, 26, 27, 28,
      ];
        ...
        <div className="booking-content">
          {data.map((booking, index) => (
            <div key={index} className="booking-item">
              ...
              <div className="house-name">{booking.houseName}</div>
